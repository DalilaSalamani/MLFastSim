...
from core.constants import N_CELLS_R, N_CELLS_Z, SIZE_R, SIZE_Z
...
class ProfileType(IntEnum):
    """IntEnum class of various profile types."""
...
class LongitudinalProfile(Profile):
    ...
    def calc_first_moment(self) -> np.ndarray:
        ...
        return (
            SIZE_Z
            * np.dot(self._energies_per_event, self._w)
    ...
    def calc_second_moment(self) -> np.ndarray:
        ...
        return (
            np.sum(
                np.multiply(
                    np.power(w * SIZE_Z - first_moment, 2), self._energies_per_event
                ),
                axis=1,
            )
            / self._total_energy_per_event
...
class LateralProfile(Profile):
    ...
    def calc_second_moment(self) -> np.ndarray:
        ...
        return (
            np.sum(
                np.multiply(
                    np.power(w * SIZE_R - first_moment, 2), self._energies_per_event
                ),
                axis=1,
            )
            / self._total_energy_per_event
...
class Energy(Observable):
    ...
    def calc_total_energy(self) -> np.ndarray:
    ...
    def calc_cell_energy(self) -> np.ndarray:
    ...
    def calc_energy_per_layer(self) -> np.ndarray:
