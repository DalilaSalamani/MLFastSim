...
from core.constants import (
    FULL_SIM_GAUSSIAN_COLOR,
    FULL_SIM_HISTOGRAM_COLOR,
    HISTOGRAM_TYPE,
    ML_SIM_GAUSSIAN_COLOR,
    ML_SIM_HISTOGRAM_COLOR,
    N_CELLS_R,
    N_CELLS_Z,
    SIZE_R,
    SIZE_Z,
...
from utils.observables import Energy, LongitudinalProfile, Profile, ProfileType
...
def _best_fit(
    data: np.ndarray, bins: np.ndarray, hist: bool = False
...
class ProfilePlotter(Plotter):
    ...
    def __post_init__(self) -> None:
        ...
        assert full_simulation_type == ml_generation_type, (
    ...
    def _plot_and_save_customizable_histogram(
        ...
        if plot_profile:
            ...
            axes[0].step(
                bins[:-1], ml_simulation, label="MLSim", color=ML_SIM_HISTOGRAM_COLOR
            )
            axes[0].vlines(
                x=bins[0],
                ymin=0,
            ...
            axes[0].vlines(
                x=bins[0], ymin=0, ymax=ml_simulation[0], color=ML_SIM_HISTOGRAM_COLOR
            )
            axes[0].vlines(
                x=bins[-2], ymin=0, ymax=ml_simulation[-1], color=ML_SIM_HISTOGRAM_COLOR
            )
        ...
        if self._plot_gaussian:
            ...
            axes[0].plot(
                xs_full_sim, ys_full_sim, color=FULL_SIM_GAUSSIAN_COLOR, label="FullSim"
            )
            axes[0].plot(
                xs_ml_sim, ys_ml_sim, color=ML_SIM_GAUSSIAN_COLOR, label="MLSim"
            )
    ...
    def _plot_first_moment(self) -> None:
        ...
        self._plot_and_save_customizable_histogram(
            full_simulation_first_moment,
            ml_simulation_first_moment,
            bins,
            xlabel,
            observable_name,
        )
    ...
    def _plot_second_moment(self) -> None:
        ...
        if self._profile_type == ProfileType.LONGITUDINAL:
            ...
            bins = np.linspace(0, pow(N_CELLS_Z * SIZE_Z, 2) / 35.0, 128)
        else:
            xlabel = "$<r^{2}> [mm^{2}]$"
            observable_name = "LatSecondMoment"
            bins = np.linspace(0, pow(N_CELLS_R * SIZE_R, 2) / 8.0, 128)

        self._plot_and_save_customizable_histogram(
            full_simulation_second_moment,
            ml_simulation_second_moment,
            bins,
            xlabel,
            observable_name,
        )
...
class EnergyPlotter(Plotter):
    ...
    def _plot_total_energy(self, y_log_scale: bool = True) -> None:
        ...
        bins = np.linspace(
            ...
            + np.max(full_simulation_total_energy) * 0.05,
            50,
        )
        plt.hist(
    ...
    def _plot_cell_energy(self) -> None:
        ...
        plt.hist(
            x=log_full_simulation_cell_energy,
            bins=bins,
            histtype=HISTOGRAM_TYPE,
            label="FullSim",
            color=FULL_SIM_HISTOGRAM_COLOR,
        ...
        plt.hist(
            x=log_ml_simulation_cell_energy,
            bins=bins,
            histtype=HISTOGRAM_TYPE,
            label="MLSim",
            color=ML_SIM_HISTOGRAM_COLOR,
    ...
    def _plot_energy_per_layer(self) -> None:
        ...
        bins = np.linspace(
            np.min(full_simulation_energy_per_layer - 10),
            np.max(full_simulation_energy_per_layer + 10),
            25,
        )
        ...
        for layer_nb in range(N_CELLS_Z):
            ...
            ax[i][j].hist(
                full_simulation_energy_per_layer[:, layer_nb],
                histtype=HISTOGRAM_TYPE,
                label="FullSim",
                bins=bins,
                color=FULL_SIM_HISTOGRAM_COLOR,
            ...
            ax[i][j].hist(
                ml_simulation_energy_per_layer[:, layer_nb],
                histtype=HISTOGRAM_TYPE,
                label="MLSim",
                bins=bins,
                color=ML_SIM_HISTOGRAM_COLOR,
